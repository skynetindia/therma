...
@section('content')
                ...
                <div class="col-md-12 col-sm-12 col-xs-12">
                    <div class="data-table">
                        ...
                        <div class="table-responsive">
                            <h1 class="cst-datatable-heading">Manage Season</h1>
                            <table data-toggle="table" data-search="true" data-pagination="true" data-id-field="id"
                                   data-show-refresh="true" data-show-columns="true"
                                   data-url="{{ url('/hotel/season/property/json')."/".$hotel_id }}"
    ...
    function confermfun(e) {
        ...
        function multipleAction(act) {
            ...
            switch (act) {
                case 'delete':
                    link.href = "{{ url('/wizard/options/delete') }}";
                    if (check() && n != 0) {
                        //for(var i = 0; i < n; i++) {
                        $.ajax({
                            type: "GET",
                            ...
                            error: function (url) {
                                if (url.status == 403) {
                                    link.href = "{{ url('/wizard/options/delete') }}" + '/' + indici;
                ...
                case 'modify':
                    if (n != 0) {
                        n--;
                        link.href = "{{ url('/wizard/option/addedit') }}" + '/' + indici;
                        n = 0;
                        selezione = undefined;
                        link.dispatchEvent(clickEvent);
                    }
