...
@section('content')
                ...
                <div class="col-md-12 col-sm-12 col-xs-12">
                    <div class="data-table">
                        <div class="table-responsive">
                            <h1 class="cst-datatable-heading">@lang('messages.keyword_category_listing')</h1><hr>
                            <table data-toggle="table" data-search="true" data-pagination="true" data-id-field="id"
                                   data-show-refresh="true" data-show-columns="true"
                                   data-url="<?php  echo url('wizard/category/property/json');?>"
                                ...
                                <th data-field="parent_id" data-sortable="true">{{trans('messages.keyword_parent_id')}}</th>
                                </thead>
                            </table>
                        </div>
                    </div>
    ...
    <script>
        function updateCategoryStatus(id) {
            var url = "{{ url('/wizard/category/changeactivestatus') }}" + '/';
            var status = '1';
        ...
        function multipleAction(act) {
            ...
            switch (act) {
                case 'delete':
                    link.href = "{{ url('/wizard/category/delete') }}" + '/';
                    if (check() && n != 0) {
                        //for(var i = 0; i < n; i++) {
                        $.ajax({
                            type: "GET",
                            ...
                            error: function (url) {
                                if (url.status == 403) {
                                    link.href = "{{ url('/wizard/category/delete') }}" + '/' + indici[n];
                ...
                case 'modify':
                    if (n != 0) {
                        n--;
                        link.href = "{{ url('/wizard/category/edit/') }}" + '/' + indici[n];
                        n = 0;
                        selezione = undefined;
                        link.dispatchEvent(clickEvent);
                    }
