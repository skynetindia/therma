    ...
    ?><input type="hidden" name="hotel_id" value="{{isset($hotelDetails->id) ? $hotelDetails->id : ''}}">
     ...
     ?><div class="section-border">
        <ul class="nav nav-tabs">            
            <li class="active"><a data-toggle="tab" href="#home">@lang('messages.keyword_sales_price')</a></li>            
        ...
        <thead>
            <tr>
                ...
                <th rowspan="2">{{trans('messages.keyword_placement')}}</th>
                <th rowspan="2">Person</th>
                <th colspan="{{$hotelMeals->count()}}">Meals</th>
            </tr>
            <tr>
        ...
        <tbody>
                ...
                <tr>
                	<td>{{(isset($vroom->personal_name) && $vroom->personal_name != "") ? ucwords(strtolower($vroom->personal_name)) : ucwords(strtolower($vroom->typename))}}</td>
                    <td><?php 
...
<div id="menu1" class="tab-pane fade"><?php 
    ...
    ?><div class="row">     
  		<div class="col-md-12 col-sm1-12 col-xs-12">        	
            <div class="record-list"><p>{{trans("messages.keyword_commission_of_therma")}}(%):  <input type="text" id="discount" value="{{$discount}}" name="discount" /></p></div>            
            <div class="data-table">
                <div class="table-responsive">
                    <table id="" class="table table-striped table-bordered" width="100%" cellspacing="0">
                        <thead>
                ...
                <tr>
                    ...
                    <?php                                                                      
                     ...
                     ?>
                    <td><input type="text" class="sales_meals_value disabled" readonly name="salemeals[{{$vroom->id}}][{{$vmeals->id}}]" value="{{$salemealprice}}" /></td>
                    @endforeach                    
                </tr><?php
                foreach($hotel_agediscount as $keyagedis => $valagedis) {
                        $extra_places = json_decode($vroom->room_extra_places,true);                                        
                            ...
                            foreach($extra_places as $keyexa => $valexa) {                                
                                if(in_array($valagedis->id, $valexa)) {
                                    ...
                                    ?><tr>
                                        ...
                                        ?><td><input type="text" class="sales_meals_value disabled" id="salesextrameals{{$vroom->id}}{{$valagedis->id}}{{$vmeals->id}}" readonly name="saleextrameals[{{$vroom->id}}][{{$valagedis->id}}][{{$vmeals->id}}]" value="{{$saleextramealprice}}" /></td>
                                        @endforeach                    
                                    </tr><?php
                                    break;
                                }
...
$('#discount').on('keyup', function (row, tr, el) {
    calculateSalesPrice();    
});
function calculateSalesPrice(){
    ...
    $(".net_price_meals").each(function( index ) {
        ...
        if(discount > 0) {
            ...
            var newval = (parseFloat($(this).val()) + parseFloat(newvalPercantage));
        }
        if(newval < 0) {
            newval = 0;
        }              
...
$('.AdultMeals').on('keyup', function (row, tr, el) {    
    calExtraDiscount();
    calculateSalesPrice(); 
});
...
function calExtraDiscount(){
     $(".extramealDiscount").each(function( index ) {     
       ...
       // Set Child Discounted Value 
        ...
        $('input[name="'+txt1+'"]').val(parseInt(newval));                          
    });
}
