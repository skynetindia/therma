...
@section('content')
    ...
    /* Start Menu based on parent-child -  dropdown */
    $menu_list = array();
    $tempm = array();
    ...
    function createTreeView($array, $currentParent, $currLevel = 0, $prevLevel = -1,&$tempm123) {
      $xx=0;
            ...
            if ($currentParent == $category['parent_id']) {                       
                    ...
                    $tempm123[$xx] = (object) array("id" => $categoryId, "name" => $dash.$category['name']);

                    if ($currLevel > $prevLevel) { $prevLevel = $currLevel; }

                    $currLevel++; 
                    //print_r( $tempm);
                    createTreeView ($array, $categoryId, $currLevel, $prevLevel,$tempm123);
   ...
   createTreeView($menu_list,0,0,-1,$tempm);
   $menus = $tempm;
   /* End Menu based on parent-child -  dropdown */
                                    ...
                                    <div class="col-md-12 col-sm-12 col-xs-12">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control selectpicker" name="user_types[]"
                                                ...
                                                @forelse(getAllUserTypes() as $key => $value)
                                                    <?php
                                                    $selected = array();
                                                    if (isset($menudetails->user_types)) {
                                                        $selected = explode(",", trim($menudetails->user_types,","));
                                                    }
                                                    ?>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="parent_id" id="parent_id"
                                                ...
                                                @foreach($menus as $key => $val)
                                                   ...
                                                   // print_r($val);
                                                    //die();
                                                    $selectedcatStatus = (isset($menudetails->parent_id) && ($val->id == $menudetails->parent_id)) ? 'selected' : ''; ?>
