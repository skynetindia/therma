...
@section('content')
    ...
    <div class="ssetting-wrap">
        ...
        <div class="row">
            <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="table-btn">
                    <a href="{{ url('template/categories/edit/') }}" class="btn btn-add"><i class="fa fa-plus"></i></a>
                ...
                <div class="col-md-12 col-sm-12 col-xs-12">
                    <div class="data-table">
                        <div class="table-responsive">
                            <h1 class="cst-datatable-heading">@lang('messages.keyword_email_template_category')</h1><hr>
                            <table data-toggle="table" data-search="true" data-pagination="true" data-id-field="id"
                                   data-show-refresh="true" data-show-columns="true"
                                   data-url="<?php  echo url('template/categories/property/json');?>"
    ...
    <script>
        function updateTemplateCategoryStatus(id) {
            var url = "{{ url('/template/categories/changeactivestatus') }}" + '/';
            var status = '1';
        ...
        function multipleAction(act) {
            ...
            switch (act) {
                case 'delete':
                    link.href = "{{ url('/template/categories/delete') }}" + '/';
                    if (check() && n != 0) {
                        //for(var i = 0; i < n; i++) {
                        $.ajax({
                            type: "GET",
                            ...
                            error: function (url) {
                                if (url.status == 403) {
                                    link.href = "{{ url('/template/categories/delete') }}" + '/' + indici[n];
                ...
                case 'modify':
                    if (n != 0) {
                        n--;
                        link.href = "{{ url('template/categories/edit/') }}" + '/' + indici[n];
                        n = 0;
                        selezione = undefined;
                        link.dispatchEvent(clickEvent);
                    }
