  ...
  <div class="row">
   	<div class="col-md-12 col-sm-12 col-xs-12">		    
        ...
        <div class="table-responsive">
          <h1 class="cst-datatable-heading">{{trans('messages.keyword_emotional_status')}}</h1>
          <table data-toggle="table" data-search="true" data-pagination="true" data-id-field="id" data-show-refresh="true" data-show-columns="true" data-url="emotionalstatus/json" data-classes="table table-bordered" id="table">
            <thead>
...
function check() { return confirm("{{trans('messages.keyword_are_you_sure_you_want_to_delete:')}}"); }
function updateEmotionalStatus(id) {
        var url = "{{ url('/emotionalstatus/changestatus') }}" + '/';
        var status = '1';
...
function multipleAction(act) {
  ...
  switch(act) {
    case 'delete':
      link.href = "{{ url('/emotionalstatus/delete/') }}" + '/';
      if(check() && n!=0) {
        for(var i = 0; i < n; i++) {
          $.ajax({
            type: "GET",
            ...
            error: function(url) {
              if(url.status==403) {
                link.href = "{{ url('/emotionalstatus/delete/') }}" + '/' + indici[n];
    ...
    case 'modify':
         if(n!=0) {
          n--;
          link.href = "{{ url('/emotionalstatus/add/') }}" + '/' + indici[n];
          n = 0;
          selezione = undefined;
          link.dispatchEvent(clickEvent);
        }
      break;
    case 'add':             
          link.href = "{{ url('/emotionalstatus/add/') }}";
          link.dispatchEvent(clickEvent);        
      break;
    }
}
