...
@section('content')
                ...
                <div class="row">
                    <div class="col-md-12 col-sm-12 col-xs-12">
                        <div class="table-btn">
                            ...
                            <a href="{{ url('package/cure-treatment/edit') }}" class="btn btn-add"><i class="fa fa-plus"></i></a>
                        ...
                        <div class="data-table">
                            <div class="table-responsive">
                                ...
                                <table id="treatment_table" data-show-refresh="true" data-show-columns="true" data-search="true" data-pagination="true" data-id-field="id" data-toggle="table" data-detail-view="true" data-detail-formatter="detailFormatter">
                                    <thead>
                                    ...
                                    <tbody>
                                        @foreach($treatments as $key => $treatment)
                                            <tr>
                                                ...
                                                <td>{{ $treatment->commission }}</td>
                                                <td>{{ $treatment->net_price }}</td>
                                                <td>{{ $treatment->is_active }}</td>
                                                <span style="display:none;" id="treat_desc{{ $key }}">
                                                    <div class="text-justify pull-left">{{ $treatment->description }}</div>
                                                    <div class="">
                                                        @if(!empty($treatment->image) && $treatment->image != null)
                                                            <img src="{{ asset('public/images/cure_treatment')."/".$treatment->image }}" class="thumbnail" style="width:60px" alt="">
                                                        @else
    ...
    <script type="application/javascript">
        ...
        /*Treatment Table*/
        var $table_treat = $('#treatment_table');

        $table_treat.on('expand-row.bs.table', function(e, index, row, $detail) {
            var res = $("#treat_desc" + index).html();
            $detail.html(res);
        });

        $table_treat.on("click-row.bs.table", function(e, row, $tr) {

            if ($tr.next().is('tr.detail-view')) {
                $table_treat.bootstrapTable('collapseRow', $tr.data('index'));
            } else {
                $table_treat.bootstrapTable('expandRow', $tr.data('index'));
            }
        });
        ...
        $('#treatment_table').on('click-row.bs.table', function (row, tr, el) {
        ...
        function multipleAction(act) {
            ...
            switch (act) {
                ...
                case 'modify':
                    if (n != 0) {
                        n--;
                        link.href = "{{ url('package/cure-treatment/edit') }}" + '/' + indici[n];
                        n = 0;
                        selezione = undefined;
                        link.dispatchEvent(clickEvent);
                    }
