...
@section('content')
        ...
        <div class="row">
            <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="table-btn">
                    ...
                    <a href="{{ url('package/options/edit') }}" class="btn btn-add"><i class="fa fa-plus"></i></a>
                ...
                <div class="col-md-12 col-sm-12 col-xs-12">
                    <div class="data-table">
                        <div class="table-responsive">
                            <h1 class="cst-datatable-heading">@lang('messages.keyword_package_options')</h1>
                            <table data-toggle="table" data-search="true" data-pagination="true" data-id-field="id"
                                   data-show-refresh="true" data-show-columns="true"
                                   data-url="{{ url('package/options/property/json') }}"
                                   data-classes="table table-bordered" id="table">
                                <thead>
                                <th data-field="id" data-sortable="true">{{trans('messages.keyword_id')}}</th>
                                <th data-field="short_name" data-sortable="true">{{trans('messages.keyword_short_name')}}</th>
                                <th data-field="image" data-sortable="true">{{trans('messages.keyword_icon')}}</th>
                                <th data-field="base_price" data-sortable="true">{{trans('messages.keyword_price')}}</th>
    ...
    <script type="application/javascript">
        ...
        function updatePackageOptionsStatus(id) {
            var url = "{{ url('/package/options/changeactivestatus') }}"  + '/';
            var status = '1';
        ...
        function check() {
            return confirm("{{trans('messages.keyword_are_you_sure_want_to_remove_package_option')}}");
        }
        ...
        function multipleAction(act) {
            ...
            switch (act) {
                case 'delete':
                    link.href = "{{ url('package/options/delete') }}" + '/';
                    if (check() && n != 0) {
                        //for(var i = 0; i < n; i++) {
                        $.ajax({
                            type: "GET",
                            ...
                            error: function (url) {
                                if (url.status == 403) {
                                    link.href = "{{ url('/package/options/delete') }}" + '/' + indici[n];
                ...
                case 'modify':
                    if (n != 0) {
                        n--;
                        link.href = "{{ url('package/options/edit') }}" + '/' + indici[n];
                        n = 0;
                        selezione = undefined;
                        link.dispatchEvent(clickEvent);
                    }
