   ...
   <div class="row">
   	<div class="col-md-12 col-sm-12 col-xs-12">		    
        ...
        <div class="table-responsive">
          <h1 class="cst-datatable-heading">{{trans('messages.keyword_location')}}</h1>
          <table data-toggle="table" data-search="true" data-pagination="true" data-id-field="id" data-show-refresh="true" data-show-columns="true" data-url="location/json" data-classes="table table-bordered" id="table">
            <thead>
	            ...
	            <!--<th data-field="zip_code" data-sortable="true">{{trans('messages.keyword_zip_code')}}</th>-->
...
function check() { return confirm("{{trans('messages.keyword_are_you_sure_you_want_to_delete:')}} "); }
 function updateTaxionStatus(id){
        var url = "{{ url('/location/changestatus') }}" + '/';
        var status = '1';
...
function multipleAction(act) {
  ...
  switch(act) {
      ...
      if(check() && n!=0) {
        for(var i = 0; i < n; i++) {
          $.ajax({
            ...
            error: function(url) {
              if(url.status==403) {
                link.href = "{{ url('/location/delete/') }}" + '/' + indici[n];
    ...
    case 'modify':
         if(n!=0) {
          n--;
          link.href = "{{ url('/location/add/') }}" + '/' + indici[n];
          n = 0;
          selezione = undefined;
          link.dispatchEvent(clickEvent);
        }
