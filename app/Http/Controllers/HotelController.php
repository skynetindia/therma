...
/*================================================================= ROOM SECTION ================================================================= */
    ...
    public function saveroom(Request $request)
    {
		...
		$ids = fetch_room_from_hotel_main($request->select_hotel, 'array');
		...
		DB::table('hotel_main')->where('id', $request->select_hotel)->update($room_ids_data);
...
/*============================================ ROOM SECTION END ================================================================== */
 ...
 /*=================================== Hotel Options Sections ========================================================== */
    ...
    public function get_room_placement_modal(Request $request) { 
		...
		$placecountqur = DB::table('room_details')->where('hotelid', $request->hotel_id)
					->where('is_deleted', '=', 0)
					->select(DB::raw('max(can_sleep) as maxroom'),DB::raw('max(extra_bed) as extrabed'))
					->first();
		
		$placecount=isset($placecountqur)?$placecountqur->maxroom:0;
      	$extraplacecount = isset($placecountqur) ? $placecountqur->extrabed : 0;     
        $ids = array(); // ids to be checked from all ids
        $html = '<table class="table table-striped"> <thead><tr><th></th>
                            <th>Room</th>' ;                           
       ...
       for($placesEx=1;$placesEx <= $extraplacecount;$placesEx++){
        ...
        foreach($roomDetails as $key=> $value) {        	
           ...
           $i = 0;
           for($placesi=1;$placesi <= $placecount;$placesi++) {
			  $disablepc=($value->can_sleep<$placesi)?'disabled="disabled"':'';
	          $html .='<td><div class="form-group"><select class="form-control" name="place_'.$value->id.'[]" '.$disablepc.'>';	          
	       ...
	       $j=0;
			for($placesi=1;$placesi <= $extraplacecount;$placesi++) {
				$disableex=($value->extra_bed < $placesi)?"disabled='disabled'":'';
	           $html .='<td><div class="form-group"><select class="form-control" name="extraplace_'.$value->id.'[]" '.$disablepc.'>';
	          foreach($ageType as $agekey => $ageval) {	          	
	          	$selectex = (isset($room_extra_places[$j]) && $room_extra_places[$j] == $ageval->id) ? 'selected' : '';	          	
    ...
    {    	
    	...
    	foreach($roomDetails as $key=> $value) {     
    		...
    		$room_extra_places = trim($room_extra_places,",");   		    		
