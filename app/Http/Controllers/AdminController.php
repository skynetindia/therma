...
class AdminController extends Controller
{
    ...
    public function storecurrency(Request $request)
    {
        ...
        if ($curreny_id && $curreny_id != "") {
            ...
            $cur = DB::table('currency')->where('id', $curreny_id)->first();
            ...
            $data = [
                ...
                'admin_default' => isset($request->admin_default) ? $request->admin_default : 0,
                'is_active' => isset($request->is_active) ? $request->is_active : 1
            ];
            
            DB::table('currency')->where('id', $curreny_id)->update($data);
            
            if ($request->admin_default == '1') {
                DB::table('currency')->update(['admin_default' => '0']);
                DB::table('currency')->where('id', $curreny_id)->update(['admin_default' => '1']);
            }
            
            if (isset($request->is_active) && $request->is_active == '0') {
                DB::table('currency')->update(['is_active' => '1']);
                DB::table('currency')->where('id', $curreny_id)->update(['is_active' => '0', 'exchange_rate' => '1']);
            }
            
            
            return Redirect::back()->with('msg', '<div class="alert alert-success"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>Currency updated successfully</div>');
            ...
            $last_inserted_id = DB::table('currency')->insertGetid($data);
            
            if ($request->admin_default == '1') {
                DB::table('currency')->update(['admin_default' => '0']);
                DB::table('currency')->where('id', $last_inserted_id)->update(['admin_default' => '1']);
            }
            
            if ($request->is_active == '0') {
                DB::table('currency')->update(['is_active' => '1']);
                DB::table('currency')->where('id', $last_inserted_id)->update(['is_active' => '0', 'exchange_rate' => '1']);
            }
    ...
    public function getjsoncurrency(Request $request)
    {
        ...
        foreach ($currency as $key => $usr) {
            ...
            if (checkpermission($this->module_id, $this->parent_id, 1)) {
                ...
                if ($usr->is_active == 0) {
                    $currency[$key]->status = '<div class="switch"><input disabled="disabled" name="status" class="currencytogal" onchange="updateTaxionStatus(' . $usr->id . ')" id="activestatus_' . $usr->id . '" ' . $checked . ' disabled on value="1"  type="checkbox"><label for="activestatus_' . $usr->id . '"></label></div>';
                } else {
                    $currency[$key]->status = '<div class="switch"><input disabled="disabled" name="status" class="currencytogal" onchange="updateTaxionStatus(' . $usr->id . ')" id="activestatus_' . $usr->id . '" ' . $checked . '  on value="1"  type="checkbox"><label for="activestatus_' . $usr->id . '"></label></div>';
                }
            }
    ...
    public function menuaddedit(Request $request)
    {
        ...
        $action = 'add';
        $menus = DB::table('dynamic_menu')->where('is_deleted', '0')->orderby('parent_id', 'asc')->orderby('priority', 'asc')->orderby('name', 'asc')->get()->toArray();
        //dd($categories);
    ...
    public function getjsonmenuproperty(Request $request)
    {
        ...
        foreach ($category as $data) {
            ...
            if (checkpermission($this->module_id, $this->parent_id, 1)) {
                ...
                $data->status = '<div  class="switch"><input disabled="disabled" name="status" class="currencytogal" onchange="updateCategoryStatus(' . $data->id . ')" id="activestatus_' . $data->id . '" ' . $checked . ' value="1"  type="checkbox"><label for="activestatus_' . $data->id . '"></label></div>';
            }
    ...
    public function get_email_template_json(Request $request)
    {
        ...
        foreach ($email_templates as $data) {
            ...
            if (checkpermission($this->module_id, $this->parent_id, 1)) {
                ...
                $data->status = '<div class="switch"><input disabled="disabled" name="status" class="currencytogal" onchange="updateEmailTemplateStatus(' . $data->id . ')" id="activestatus_' . $data->id . '" ' . $checked . ' value="1"  type="checkbox"><label for="activestatus_' . $data->id . '"></label></div>';
            }
...
//            }
    ...
    public function get_email_template_category_json(Request $request)
    {
        ...
        foreach ($email_template_categorys as $data) {
            ...
            if (checkpermission($this->module_id, $this->parent_id, 1)) {
                ...
                $data->status = '<div class="switch"><input disabled="disabled" name="status" class="currencytogal" onchange="updateTemplateCategoryStatus(' . $data->id . ')" id="activestatus_' . $data->id . '" ' . $checked . ' value="1"  type="checkbox"><label for="activestatus_' . $data->id . '"></label></div>';
            }
    ...
    /*Taxonomies Discount Action*/
    public function taxonomiesdiscount_action(Request $request)
    {
        if (!checkpermission($this->module_id, $this->parent_id, 0)) {
            return redirect('/unauthorized');
        }
        
        return view('settings.taxonomies_discount_action', [
            'taxinomies_discount_action' => DB::table('taxonomies_discount_action')->orderBy('id', 'desc')->get(),
        ]);
    }
    
    public function savediscount_actiontaxonomies(Request $request)
    {
        $languageKey = 'keyword_' . str_replace(" ", "_", strtolower($request->name));
        ...
        DB::table('taxonomies_discount_action')->insert([
            'name' => $request->name,
            'color' => $request->color,
            'language_key' => $languageKey,
            'user_id' => $request->user()->id
        ]);
        return Redirect::back()->with('msg', '<div class="alert alert-success"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a> ' . trans('messages.keyword_discount_action_added_successfully') . ' </div>');
    }
    
    /* This function is used to update/delete */
    public function discount_actiontaxonomiesUpdate(Request $request)
    {
        
        foreach ($request->chkdiscount_action as $key => $val) {
            $name = isset($request->name[$key]) ? $request->name[$key] : '';
            $languageKey = 'keyword_' . str_replace(" ", "_", strtolower($request->name[$key]));
            $color = isset($request->color[$key]) ? $request->color[$key] : '';
            ...
            if ($request->action == 'delete') {
                DB::table('taxonomies_discount_action')->where('id', $key)->delete();
            } else {
                DB::table('taxonomies_discount_action')->where('id', $key)->update(array('name' => $name, 'color' => $color, 'language_key' => $languageKey));
            }
        }
        $msg = ($request->action == 'delete') ? '<div class="alert alert-danger"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a> ' . trans('messages.keyword_discount_action_removed_successfully') . ' </div>' : '<div class="alert alert-success"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a> ' . trans('messages.keyword_discount_action_updated_successfully') . ' </div>';
        return Redirect::back()->with('msg', $msg);
    }
    /*Taxonomies Discount Action*/
    ...
    public function taxonomies_payment(Request $request)
    {
        ...
        return view('settings.taxonomies_payment', [
            ...
            'taxonomies_invoice_actions' => DB::table('taxonomies_invoice_action')->orderBy('id', 'desc')->get(),
        ]);
    }
    ...
    public function saveinvoice_actiontaxonomies(Request $request)
    {
        $languageKey = 'keyword_' . str_replace(" ", "_", strtolower($request->name));
        ...
        DB::table('taxonomies_invoice_action')->insert([
            'name' => $request->name,
            'description' => $request->description,
            'color' => $request->color,
            'language_key' => $languageKey,
        ...
        return Redirect::back()->with('msg', '<div class="alert alert-success"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a> ' . trans('messages.keyword_invoice_action_added_successfully') . ' </div>');
    }
    
    /* This function is used to update/delete */
    public function invoice_actiontaxonomiesUpdate(Request $request)
    {
//pre($request->all());
        
        foreach ($request->chkinvoice_action as $key => $val) {
            $name = isset($request->name[$key]) ? $request->name[$key] : '';
            $description = isset($request->description[$key]) ? $request->description[$key] : '';
            $languageKey = 'keyword_' . str_replace(" ", "_", strtolower($request->name[$key]));
            $color = isset($request->color[$key]) ? $request->color[$key] : '';
            ...
            if ($request->action == 'delete') {
                DB::table('taxonomies_invoice_action')->where('id', $key)->delete();
            } else {
                DB::table('taxonomies_invoice_action')->where('id', $key)->update(array('name' => $name, 'description' => $description, 'color' => $color, 'language_key' => $languageKey));
            }
        }
        $msg = ($request->action == 'delete') ? '<div class="alert alert-danger"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a> ' . trans('messages.keyword_invoice_action_removed_successfully') . ' </div>' : '<div class="alert alert-success"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a> ' . trans('messages.keyword_invoice_action_updated_successfully') . ' </div>';
        return Redirect::back()->with('msg', $msg);
    }
    /*Taxonomies Payment*/
    ...
    public function cms_modify(Request $request)
    {
        if (!checkpermission($this->module_id, $this->parent_id, 1)) {
            return redirect('/unauthorized');
        }
        ...
        $data = ['action' => 'add'];
        if (isset($request->cms_id)) {
            $data['action'] = 'edit';
            $data['cms'] = DB::table('cms')->where('id', $request->cms_id)->first();
        }
        
        
        $data['language_transalation'] = DB::table('language_transalation')->where('id', $request->id)->first();
        $data['language'] = DB::table('languages')->where('is_deleted', '0')->get();
        $data['language_selected'] = DB::table('languages')->where('code', 'en')->first();
        
        return view('cms.cms_modify', $data);
    }
    ...
    public function cms_update(Request $request)
    {
        ...
        //update cms
        if (isset($request->cms_id) && !empty($request->cms_id) && $request->action == 'edit') {
            
            $cms_to_update = DB::table('cms')->where('id', $request->cms_id)->first();
            
            $title = $request['title'];
            $title_slug = slugify($title);
            $title_lang_key = 'keyword_' . str_replace(" ", "_", strtolower($title_slug));
            //$title_lang_key = checkUniqueTitle($title_lang_key);
            
            
            $cms_desc_language_key = (isset($request['title']) && !empty($request['title'])) ? 'keyword_' . str_replace(" ", "_", strtolower($title_slug . '_cms_desc')) : null;
            $seo_title_language_key = (isset($request['title']) && !empty($request['title'])) ? 'keyword_' . str_replace(" ", "_", strtolower($title_slug . '_seo_title')) : null;
            $og_title_language_key = (isset($request['title']) && !empty($request['title'])) ? 'keyword_' . str_replace(" ", "_", strtolower($title_slug . '_og_title')) : null;
            $seo_description_language_key = (isset($request['title']) && !empty($request['title'])) ? 'keyword_' . str_replace(" ", "_", strtolower($title_slug . '_seo_description')) : null;
            $og_description_language_key = (isset($request['title']) && !empty($request['title'])) ? 'keyword_' . str_replace(" ", "_", strtolower($title_slug . '_og_description')) : null;
            $keywords_language_key = (isset($request['title']) && !empty($request['title'])) ? 'keyword_' . str_replace(" ", "_", strtolower($title_slug . '_keywords')) : null;
            $og_url_language_key = (isset($request['title']) && !empty($request['title'])) ? 'keyword_' . str_replace(" ", "_", strtolower($title_slug . '_og_url')) : null;
            
            
            foreach ($arrLanguages as $key => $val) {
                if (isset($request[$val->code . '_cms_desc'])) {
                    $cms_desc_language_value = $request[$val->code . '_cms_desc'];
                    DB::table('language_transalation_test')
                        ->where(['code' => $val->code, 'language_key' => $cms_to_update->description, 'language_label' => $cms_to_update->title_language_key])
                        ->update([
                            'language_key' => $cms_desc_language_key,
                            'language_label' => $title_lang_key,
                            'language_value' => $cms_desc_language_value,
                            'code' => $val->code
                        ]);
                }
                if (isset($request[$val->code . '_seo_title'])) {
                    $seo_title_language_value = $request[$val->code . '_seo_title'];
                    DB::table('language_transalation_test')
                        ->where(['code' => $val->code, 'language_key' => $cms_to_update->seo_title, 'language_label' => $cms_to_update->title_language_key])
                        ->update([
                            'language_key' => $seo_title_language_key,
                            'language_label' => $title_lang_key,
                            'language_value' => $seo_title_language_value,
                            'code' => $val->code
                        ]);
                }
                if (isset($request[$val->code . '_og_title'])) {
                    $og_title_language_value = $request[$val->code . '_og_title'];
                    DB::table('language_transalation_test')
                        ->where(['code' => $val->code, 'language_key' => $cms_to_update->og_title, 'language_label' => $cms_to_update->title_language_key])
                        ->update([
                            'language_key' => $og_title_language_key,
                            'language_label' => $title_lang_key,
                            'language_value' => $og_title_language_value,
                            'code' => $val->code
                        ]);
                }
                if (isset($request[$val->code . '_seo_description'])) {
                    $seo_description_language_value = $request[$val->code . '_seo_description'];
                    DB::table('language_transalation_test')
                        ->where(['code' => $val->code, 'language_key' => $cms_to_update->seo_description, 'language_label' => $cms_to_update->title_language_key])
                        ->update([
                            'language_key' => $seo_description_language_key,
                            'language_label' => $title_lang_key,
                            'language_value' => $seo_description_language_value,
                            'code' => $val->code
                        ]);
                }
                if (isset($request[$val->code . '_og_description'])) {
                    $og_description_language_value = $request[$val->code . '_og_description'];
                    
                    DB::table('language_transalation_test')
                        ->where(['code' => $val->code, 'language_key' => $cms_to_update->og_description, 'language_label' => $cms_to_update->title_language_key])
                        ->update([
                            'language_key' => $og_description_language_key,
                            'language_label' => $title_lang_key,
                            'language_value' => $og_description_language_value,
                            'code' => $val->code
                        ]);
                }
                if (isset($request[$val->code . '_keywords'])) {
                    $keywords_language_value = $request[$val->code . '_keywords'];
                    DB::table('language_transalation_test')
                        ->where(['code' => $val->code, 'language_key' => $cms_to_update->keywords, 'language_label' => $cms_to_update->title_language_key])
                        ->update([
                            'language_key' => $keywords_language_key,
                            'language_label' => $title_lang_key,
                            'language_value' => $keywords_language_value,
                            'code' => $val->code
                        ]);
                }
                if (isset($request[$val->code . '_og_url'])) {
                    $og_url_language_value = $request[$val->code . '_og_url'];
                    DB::table('language_transalation_test')
                        ->where(['code' => $val->code, 'language_key' => $cms_to_update->og_url, 'language_label' => $cms_to_update->title_language_key])
                        ->update([
                            'language_key' => $og_url_language_key,
                            'language_label' => $title_lang_key,
                            'language_value' => $og_url_language_value,
                            'code' => $val->code
                        ]);
                }
            }
            ...
            $cms_data = [
                'title' => $title,
                'title_language_key' => $title_lang_key,
                'description' => $cms_desc_language_key,
                'seo_title' => $seo_title_language_key,
                'og_title' => $og_title_language_key,
                'seo_description' => $seo_description_language_key,
                'og_description' => $og_description_language_key,
                'keywords' => $keywords_language_key,
                'og_url' => $og_url_language_key,
                'updated_at' => date('Y-m-d H:i:s')
            ];
            DB::table('cms')->where('id', $request->cms_id)->update($cms_data);
            
            $msg = '<div class="alert alert-success"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>' . trans('messages.keyword_cms_updated_successfully') . '</div>';
        } else {
            //insert cms
            ...
            foreach ($arrLanguages as $key => $val) {
                if (isset($request[$val->code . '_cms_desc'])) {
                    $cms_desc_language_value = $request[$val->code . '_cms_desc'];
                    DB::table('language_transalation_test')->insert([
                        'language_key' => $cms_desc_language_key,
                        'language_label' => $title_lang_key,
                        'language_value' => $cms_desc_language_value,
                        'code' => $val->code
            ...
            DB::table('cms')->insert($cms_data);
            
            $msg = '<div class="alert alert-success"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>' . trans('messages.keyword_cms_added_successfully') . '</div>';
        }
    ...
    public function getjsoncms(Request $request)
    {
        $cmsdetails = array();
        $category = DB::table('cms')->select('*')->where('id', '!=', 0)->where('is_deleted', '=', 0)->get();
        foreach ($category as $data) {
            $checked = ($data->is_active == 1) ? 'checked' : '';
            
            if (checkpermission($this->module_id, $this->parent_id, 1)) {
                $data->status = '<div class="switch"><input name="status" class="currencytogal" onchange="updateCMSstatus(' . $data->id . ')" id="activestatus_' . $data->id . '" ' . $checked . ' value="1"  type="checkbox"><label for="activestatus_' . $data->id . '"></label></div>';
            } else {
                $data->status = '<div class="switch"><input disabled="disabled" name="status" class="currencytogal" onchange="updateCMSstatus(' . $data->id . ')" id="activestatus_' . $data->id . '" ' . $checked . ' value="1"  type="checkbox"><label for="activestatus_' . $data->id . '"></label></div>';
            }
        ...
        return json_encode($cmsdetails);
    }
    
    public function updatecmsstatus(Request $request)
    {
        $update = DB::table('cms')->where('id', $request->id)->update(array('is_active' => $request->status));
        return ($update) ? 'true' : 'false';
    }
    
    public function deletecms(Request $request)
    {
        $countRec = DB::table('cms')->select('*')->where('id', $request->id)->count();
        if ($countRec > 0) {
            DB::table('cms')->where('id', $request->id)->update(array('is_deleted' => '1'));
            return Redirect::back()->with('msg', '<div class="alert alert-danger"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>' . trans('messages.keyword_cms_removed_successfully') . '</div>');
        } else {
            return Redirect::back()->with('msg', '<div class="alert alert-danger"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>' . trans('messages.keyword_cms_not_exist') . '</div>');
        }
    }
    ...
    {
        $check = DB::table('invoice_detail')->where('id', 1)->first();
        
        $data['invoice_detail'] = '';
        if(count($check) > 0){
            $data['invoice_detail'] = $check;
        }
        
    
        return view('settings.invoice_detail', $data);
    }
    
    public function invoice_detail_save(Request $request)
    {
        $check = DB::table('invoice_detail')->where('id', 1)->first();
        
        $iban = (count($request->iban) > 0) ? implode(',', $request->iban) : '';
        $vat_id = (count($request->vat_id) > 0) ? implode(',', $request->vat_id) : '';
        $swift = (count($request->swift) > 0) ? implode(',', $request->swift) : '';
        ...
        if (count($check) > 0) {
            ...
            $update = DB::table('invoice_detail')->where('id', 1)->update($data);
        } else {
            ...
            DB::table('invoice_detail')->insertGetId($data);
        }
        return Redirect::back();
    }
    /* Invoice Detail */
